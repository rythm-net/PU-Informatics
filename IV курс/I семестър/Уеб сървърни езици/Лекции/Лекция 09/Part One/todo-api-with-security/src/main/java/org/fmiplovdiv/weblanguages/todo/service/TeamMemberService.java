...
public class TeamMemberService {
	...
	public List<TeamMemberResponse> getAllTeamMembers() {
		...
		return toReturn.stream()
				.map(t -> new TeamMemberResponse(t.getId(), t.getName()))
				.toList();
	}
	...
	public TodoResponse getCurrentTodo(Long id) {
		...
		Todo currentTodo = todoRepository
				.findAllByTeamMemberAndTodoStatus(
						teamMember, 
		...
		return new TodoResponse(
				...
				currentTodo.getTodoStatus().ordinal(), 
				currentTodo.getTeamMember().getId()
				);
	}
	
	@Transactional
	public List<TodoResponse> getAllTodosByTeamMemberAndTodoStatus(
			Long id,
		...
		List<Todo> todos = todoRepository
				.findAllByTeamMemberAndTodoStatus(teamMember, todoStatus);
