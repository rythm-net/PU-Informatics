...
const SpotifyClient = {
  ...
  getAlbum(albumId) {
    return this._get(SPOTIFY_BASE_URI + "/albums/" + albumId).then((data) =>
      parseAlbum(data)
    );
  },
  ...
  getAlbums(albumIds) {
    ...
    ).then((data) => data.albums.map((a) => parseAlbum(a)));
  },

  getArtist(artistId) {
    return this._get(SPOTIFY_BASE_URI + "/artists/" + artistId).then((data) =>
      parseArtist(data)
    );
  },
